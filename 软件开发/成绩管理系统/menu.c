...
void main_menu(StudentList *list, User *current_user, UserNode *user_head) {
    ...
    while (1) {
        ...
        choice = get_int_input("请选择", 0, max_choice);
...
static void handle_add_student(StudentList *list) {
    ...
    printf("\n--- 添加学生 ---\n");
    get_string_input("学号", stu.id, sizeof(stu.id));
    ...
    get_string_input("姓名", stu.name, sizeof(stu.name));
    get_string_input("性别 (男/女)", stu.gender, sizeof(stu.gender));
    get_string_input("班级", stu.class_name, sizeof(stu.class_name));
...
static void handle_delete_student(StudentList *list) {
    char id[20];
    get_string_input("请输入要删除的学号", id, sizeof(id));
...
static void handle_update_student(StudentList *list) {
    char id[20];
    get_string_input("请输入要修改的学号", id, sizeof(id));
    ...
    printf("性别 [%s]: ", stu.gender);
    if (fgets(buf, sizeof(buf), stdin) && buf[0] != '\n') {
        buf[strcspn(buf, "\n")] = '\0';
        strncpy(stu.gender, buf, sizeof(stu.gender) - 1);
    }
    printf("班级 [%s]: ", stu.class_name);
    if (fgets(buf, sizeof(buf), stdin) && buf[0] != '\n') {
        buf[strcspn(buf, "\n")] = '\0';
...
static void handle_find_by_id(StudentList *list) {
    char id[20];
    get_string_input("请输入学号", id, sizeof(id));
...
static void handle_find_by_name(StudentList *list) {
    char name[50];
    get_string_input("请输入姓名（支持模糊查找）", name, sizeof(name));
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 4: {
                ...
                int sub = get_int_input("请选择科目", 1, SUBJECT_COUNT) - 1;
...
static void handle_update_score(StudentList *list) {
    ...
    int sub = get_int_input("请选择", 1, SUBJECT_COUNT) - 1;
    float score = get_float_input("新成绩", 0, 100);
...
static void handle_class_stats(StudentList *list) {
    char cls[30];
    get_string_input("请输入班级名称", cls, sizeof(cls));
...
static void handle_restore_data(StudentList **list) {
    ...
    if (confirm_action("恢复将覆盖当前数据，确认？")) {
        list_destroy(*list);
        *list = list_create();
        load_students_from_file(*list, path);
        calc_total_and_average(*list);
...
static void menu_user_manage(UserNode **user_head) {
    ...
    while (1) {
        ...
        choice = get_int_input("请选择", 0, 2);
        switch (choice) {
            case 1: {
                User user;
                memset(&user, 0, sizeof(User));
                get_string_input("用户名", user.username, sizeof(user.username));
                ...
                get_string_input("密码", user.password, sizeof(user.password));

                printf("角色: 0-管理员 1-教师 2-学生\n");
                user.role = (UserRole)get_int_input("请选择角色", 0, 2);

                if (user.role == ROLE_STUDENT) {
                    get_string_input("关联学号", user.bind_id, sizeof(user.bind_id));
                } else if (user.role == ROLE_TEACHER) {
                    get_string_input("教师编号", user.bind_id, sizeof(user.bind_id));
                }
